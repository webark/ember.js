...
function bind(self, funcName) {
  let func = self[funcName];
...
class LifeCycleHooksTest extends RenderingTest {
  ...
  get boundHelpers() {
    return {
      invoke: bind(this, 'invocationFor'),
      attr:   bind(this, 'attrFor')
    };
  }
  ...
  assertHooks(...rawHooks) {
    let hooks = rawHooks.map(raw => hook(...raw));
    this.assert.deepEqual(this.hooks, hooks);
    this.hooks = [];
  }
